...
| `/help` | Справка |
| `/insert 1245`<br>`<текст>` | Добавить наблюдение по указанному УИК напрямую в базу — только в личке с ботом (в топике не работает), только `ADMIN_ID`, без подтверждения |
...
Единственное исключение — `/insert`: это тот же механизм наблюдений, просто
введённый вручную администратором за произвольный УИК в личной переписке с
ботом, а не в общем топике.
